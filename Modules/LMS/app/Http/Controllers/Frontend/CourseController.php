...
class CourseController extends Controller
{
    ...
    public function courseDetail($slug)
    {
        ...
        return view('theme::course.course-detail', compact('course', 'relatedCourses'));
    }

    /**
     * courseBundle
     */
    public function courseBundle(Request $request)
    {
        $bundles = $this->course->courseBundle(request: $request, item: 4);
        if ($request->ajax()) {
            $result = view('theme::course.bundle-list', compact('bundles'))->render();
            return response()->json(
                [
                    'status' => true,
                    'data' => $result,
                    'total' => $bundles->total(),
                    'first_item' => $bundles->firstItem(),
                    'last_item' => $bundles->lastItem(),
                ]
            );
        }
