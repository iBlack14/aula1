            ...
            <div
                ...
                <div class="relative aspect-video rounded-xl overflow-hidden">
                    ...
                    @auth
                        ...
                        @endphp
                        <label for="course_{{ $course->id . '' . $randomKey }}"
                            class="flex-center absolute top-3 right-4 rtl:right-auto rtl:left-4 size-11 rounded-50 bg-white cursor-pointer select-none z-[1] add-wishlist group/wishlist {{ $class }}"
                            data-id="{{ $course->id }}">
                            <input type="checkbox" id="course_{{ $course->id . '' . $randomKey }}"
                    ...
                    @else
                        <label for="course_{{ $course->id . '' . $randomKey }}"
                            class="flex-center absolute top-3 right-4 rtl:right-auto rtl:left-4 size-11 rounded-50 bg-white cursor-pointer select-none z-[1]"
                            data-id="{{ $course->id }}">
                            <a href="{{ route('auth.login') }}" id="course_{{ $course->id . '' . $randomKey }}"
                ...
                <div class="mt-6 group-data-[card-layout=list]:mt-0 group-data-[card-layout=list]:ml-6">
                    ...
                    <h6 class="area-title font-bold !text-xl mt-3 group-hover/course:text-primary custom-transition">
                        <a href="{{ route('course.detail', $course->slug) }}" class="line-clamp-2"
                            aria-label="Course title">{{ $translations['title'] ?? ($course->title ?? '') }}</a>
                    </h6>
